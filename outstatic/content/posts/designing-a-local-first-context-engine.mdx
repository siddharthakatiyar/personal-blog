...
When a repository is arbitrarily sliced into 1,000-character chunks, the structural integrity of the code is lost. An LLM might receive the bottom half of a class definition, a random import block from another file, and a loosely related configuration snippet. The execution flow is fragmented, forcing the model to hallucinate the missing connections.
...
Crucially, it did this while averaging just **669 tokens per query**. 

Token efficiency directly translates to lower latency, reduced API costs, and significantly less model confusion. A model analyzing ~670 highly relevant tokens will consistently outperform a model drowning in 40,000 tokens of noisy, full-file context.
